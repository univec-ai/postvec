...
pub fn stage_from_archive(
    ...
    crate::config::owned::create_dir_all_checked(&staged, 0o755)?;
...
fn set_staged_enabled(staged: &Path, enabled: bool) -> Result<()> {
    ...
    body.push(b'\n');
    crate::config::owned::write_atomic(&path, &body, 0o644)
}
...
mod tests {
    use super::*;
    use crate::registry::archive::{write_deterministic, ArchiveInput};
