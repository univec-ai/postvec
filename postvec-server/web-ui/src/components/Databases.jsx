...
export default function Databases() {
  ...
  return <section className="workbench">
    ...
    {!databases.length && <p>No managed databases configured. Add a managed block to the server configuration or use serve --sync.</p>}
    {databases.map((db) => {
      const d = db.database || {}
      const age = d.heartbeat_age_seconds
      return <article key={db.name} className="result-card">
        ...
        <button disabled={busy} onClick={() => request(db.name, 'jobs')}>Inspect jobs</button>{' '}
        <button disabled={busy} onClick={() => request(db.name, 'refresh-models')}>Refresh models</button>
      </article>
    })}
    {detail && <section><h2>Jobs · {detail.db}</h2>
      ...
      {(detail.dead || []).map((j) => <p key={j.dead_id}>Dead job {j.dead_id}: {j.last_error}{' '}
        <button disabled={busy} onClick={() => request(detail.db, 'retry-dead', { registry_id: j.registry_id, dead_ids: [j.dead_id] })}>Retry</button>
      </p>)}
    </section>}
  </section>
}
