...
pub trait RegistryEntryDb: Sized {
    const SELECT: &'static str = "SELECT id, table_schema, table_name, source_column,
                vector_column, pk_columns, pk_types, model, space, dim,
...
impl RegistryEntryDb for RegistryEntry {
    ...
    fn from_row(row: &pgrx::spi::SpiHeapTupleData) -> Self {
        RegistryEntry {
            ...
            space: row.get::<String>(9).unwrap(),
            dim: row.get::<i32>(10).unwrap().unwrap(),
            fts_config: row.get::<String>(11).unwrap().unwrap(),
            distance: row.get::<String>(12).unwrap().unwrap(),
            backfill_mode: row.get::<String>(13).unwrap().unwrap(),
            backfill_watermark: row.get::<String>(14).unwrap(),
            state: row.get::<String>(15).unwrap().unwrap(),
            trigger_mode: row.get::<String>(16).unwrap().unwrap(),
            owns_vector_column: row.get::<bool>(17).unwrap().unwrap(),
            format: row.get::<String>(18).unwrap(),
            index_mode: row.get::<String>(19).unwrap().unwrap(),
            index_error: row.get::<String>(20).unwrap(),
            chunking: row.get::<String>(21).unwrap().unwrap(),
            chunk_size: row.get::<i32>(22).unwrap(),
            chunk_overlap: row.get::<i32>(23).unwrap(),
            destination_schema: row.get::<String>(24).unwrap(),
            destination_table: row.get::<String>(25).unwrap(),
            destination_view: row.get::<String>(26).unwrap(),
            destination_token: row.get::<String>(27).unwrap(),
        }
    }
    fn load(id: i64) -> Option<RegistryEntry> {
