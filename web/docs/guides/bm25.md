...
`search()` fuses two retrieval legs with Reciprocal Rank Fusion:

- semantic - nearest neighbours in the vector column
- lexical - BM25 over PostgreSQL text search (`fts_score`)

Both legs run on every `search()` call. `semantic_weight` sets how much each rank contributes to `rrf_score`.

## Enable the GIN index

BM25 scoring is part of `search()`. The GIN index is what makes the lexical leg fast on a large table. Pass `create_fts_index => true` at [`enable()`](/docs/guides/enable) or [`adopt()`](/docs/guides/adopt):
