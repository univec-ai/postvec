...
| Inference on a separate host, GPU or a fleet | [Install postvec-server](/docs/server/node), then [connect PostgreSQL](/docs/server/connect). Both containers: [quick start remote](/docs/quickstart-remote) |
| Replacing a pgai / pg_vectorize pipeline | [Coming from pgai](/docs/from-pgai) |
