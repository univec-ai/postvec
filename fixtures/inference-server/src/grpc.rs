//! The fixture's gRPC server, lifted from postvec's embedded loopback
//! server (postvec/src/client/embedded/server.rs).
//!
//! Serves the canonical ninference proto with the same payload shapes into
//! `predict_raw` and the same `x-ravenna-error-code` metadata out, so a
//! postvec tested against this fixture classifies errors exactly as it
//! would against a production node.
...
/// Bind and serve. Unlike postvec's embedded loopback server this may bind a
/// non-loopback address: the fixture serves a private Docker test network and
/// is never published — it exists only to be dialed by the smoke test's
/// postvec container.
pub async fn serve(
    engine: Arc<InferenceEngine>,
    addr: SocketAddr,
    ...
    log::info!("fixture gRPC listening on {bound}");
